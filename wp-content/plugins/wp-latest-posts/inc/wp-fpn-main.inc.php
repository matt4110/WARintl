...
//		);
	...
	public function editorTabs() {
		...
		<div style="background:#fff" class="ui-tabs ui-widget ui-widget-content ui-corner-all">
			...
			<div id="wpcufpnnavtabs" class="wpcufpntabs" style="display: none">
				<ul>
					<li><a href="#tab-1"><?php _e( 'Content source', 'wp-latest-posts' ); ?></a></li>
					<li><a href="#tab-2"><?php _e( 'Display and theme', 'wp-latest-posts' ); ?></a></li>
					<li><a href="#tab-3"><?php _e( 'Images source', 'wp-latest-posts' ); ?></a></li>
					<li><a href="#tab-4"><?php _e( 'Advanced', 'wp-latest-posts' ); ?></a></li>
				</ul>
	...
	private function displayDisplayThemeTab() {
		...
		/** Theme drop-down **/
		echo '<li class="field"><label for="theme" class="coltab">' . __( 'Theme', 'wp-latest-posts' ) . '</label>' .
	...
	private function displayImageSourceTab() {
		...
		echo '<li class="field"><label for="thumb_img" class="coltab">' . __( 'Select Image', 'wp-latest-posts' ) . '</label>' .
		...
		/** image Size field **/
		echo '<li class="field"><label for="thumb_width" class="coltab">' . __( 'Image size', 'wp-latest-posts' ) . '</label>' .
	...
	private function displayAdvancedTab() {
		...
		if( !class_exists('wpcuWPFnProPlugin') ) {
			...
			echo '<p>' . __('Looking out for more <em>advanced</em> features?','wp-latest-posts') . '</p>';
			echo '<p>' . __('&rarr;&nbsp;Check out our optional <a href="http://www.joomunited.com/wordpress-products/wp-latest-posts" target="_blank" >"Pro" add-on</a>.','wp-latest-posts') . '</p>';
			echo '</div>';
		} else {
        ...
        {
            echo '<hr/><div><label for="phpCodeInsert" class="coltab" style="margin:10px 0 5px">' . __( 'Copy & paste this code into a template file to display this WPLP block', 'wp-latest-posts' ) . '</label>' .
	...
	public function displayAboutTab() {
		...
		if( !class_exists('wpcuWPFnProPlugin') ) {
			echo '<div id="promote">';
			echo '<h1>'.__('Get Pro version', 'wp-latest-posts').'</h1>';			
